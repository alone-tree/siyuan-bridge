...
## 2026-09-25：set_tags 编辑文档标签

- 归属定为 `siyuan_doc_manage` 新 action `set_tags`（文档级事实变更，语义同 rename：`read_write` + `confirmed=true` + 写前快照 + 写后 pushMsg 和安全刷新索引），不放 `siyuan_edit`（正文块工具，标签无块坐标）。
- 底层实测：`/api/attr/setBlockAttrs` 设置文档块 `tags` 属性即可，值必须是半角逗号分隔、不带 `#` 的裸名；空值清除；内核按半角逗号拆分，空格/顿号/分号/全角逗号在名称内合法；SQL `tag` 列有秒级索引延迟。
- 输入解析（`parse_tags_input()`）：严格解析，只认成对半角 `#`；提取后剩余字符必须是分隔符白名单（空白、`,` `，` `、` `;` `；`），裸名、未闭合 `#`、孤立 `#`、全角 `＃` 包裹一律报错；名称含官方特殊符号清单（`` ` * _ [ ] ! \ < > & ~ $ ( ) { } = #``，官方用户指南「标签」章节）或半角逗号时报错并返回完整清单；空名称报错；去重保序；空串清除全部。工具描述只教 `tags="#标签1# #标签2#"` 空格分隔写法。
- 返回：成功信息 + 标签数量 + 逐个 `#名称#`；清除时返回「已清除全部文档标签（0 个）」。需要增删单个标签时由 AI 先读当前标签再整体写回。
- 字符规则依据：官方用户指南「标签」章节（层级标签 `#A/B/C#`、特殊符号「请勿」清单）+ 内核实测（逗号拆分、其余字符保留）。
- 测试：`python -m pytest tests -q` 为 `397 passed, 1 skipped`（新增解析边界 5 例 + set_tags 集成 6 例）。开发版 MCP 实调：混合分隔符 `#测试标签E#，#层级/测试# #测试标签F#` 设置成功（快照+索引刷新），read 头部读回 `tag：#测试标签E# #层级/测试# #测试标签F#`；`#甲(1)#` 被拒并返回完整禁止清单；空串清除成功。
- 版本仍为 1.9.0，尚未发布。
...
- 这是查询体验修复，按 PATCH 从 1.7.2 升到 1.7.3；同版本还包含多词 AND 提示和默认两层验证规则。

## 2026-08-26：明确 siyuan_find 多词查询的 AND 语义（v1.7.3）
...
- 单条通知最多显示两行，保持通知区高度稳定。

## 2026-08-01：v1.5.1 系统笔记本单写入入口与重复文档兼容

- 系统笔记本维护统一到插件激活阶段；安装、更新、启动思源或重新启用插件时自动执行。`siyuan_start`、安全索引刷新和写后刷新只读已登记状态，不再创建、更新、迁移系统文档或写 `system_state.json`。
- `system_state.json` 升级为 schema v2，每种文档保存多个条目。插件激活合并仍有效的 JSON ID、当前名称和历史名称匹配结果，移除失效 ID；只有某类型候选为零时才创建。
- 多篇同类型文档全部登记并合并使用，不自动删除或提供一次性修复。插件弹窗提示用户手动检查删除；用户删除后未重载插件，`siyuan_start` 会实时跳过失效 ID，只要仍有一篇有效文档就正常继续。
- 非 Privacy Rules 类型全部失效时，向思源推送 warning 并在启动包注入提示，仍继续运行；Privacy Rules 全部失效时失败关闭，提示用户禁用并重新启用插件。多篇 Privacy Rules 全部硬隔离并合并解析。
- 补充旧式标题 `MCP使用指南` 的兼容扫描。测试空间 v1.5.0 现场确有该标题；v1 状态迁移到 v2 后六类文档 ID 全部保留，没有新建重复文档。
- 重复场景真实验证：创建第二篇空白“用户个性化要求”后重载，JSON 登记 2 个 ID；删除测试文档但不重载时，`siyuan_start` 成功且不创建文档；再次重载后失效 ID 自动清理为 1，现场只剩原文档。测试临时文档已删除。
- 验证：完整测试 307 passed；插件 JS 语法、Python compileall、diff check 通过；当前测试空间 JSON-RPC 探针、能力库临时 MCP、独立子代理三层均通过。临时能力库条目已关闭并删除。
...
- 创建前读取思源 live 笔记本列表并按不区分大小写的名称拒绝重复创建；目标名称受 Privacy Rules 限制时拒绝写入。
- `siyuan_create` 和复制目标路径找不到笔记本时，会明确提示 AI 先调用 create_notebook；普通文档路径仍不会隐式创建笔记本。
- `delete` 继续只删除文档子树，当前不提供删除整个笔记本的 action。
- 工具版本升级为 `1.4.0`。
- 真实开发版 MCP 验证：创建两个带时间戳的测试笔记本均成功并自动刷新索引；重复创建同名笔记本在快照前拒绝；随后在第一个测试笔记本中创建并读回测试文档成功。三次实际写入均返回快照已创建。

## 2026-07-29：v1.3.0 主动引用检测

- `siyuan_operate` 新增只读 `action="check_references"`，复用现有 `document` / `document_id` 定位器；检测当前文档 ID 和全部真实正文块 ID，不依赖引用阅读视图中是否单独展示列表子项等块。
- 底层统一合并 `refs` 中的标准块引用/可识别嵌入块与 `spans` 中的 `siyuan://blocks/<ID>` 块链接，并按 `(目标 ID, 来源块 ID)` 去重。主动查询和被动删除保护复用同一关系查询，因此删除保护同步覆盖三种关系。
- 当前文档按来源文档汇总并展示引用块原始 Markdown；每篇最多 3 个引用块，单块最多 2000 字符。子文档递归统计但只展示可见子文档的次数；隐藏来源和隐藏子文档仅按已确认的聚合规则计数。
- `limit` 默认 10、最小 1、无整数上限，支持 `"none"`；总数不受展示上限影响。该 action 不要求确认，不创建快照。
- 工具版本升级为 `1.3.0`。历史 `v1.0.5` 的功能规模实际对应 `v1.1.0`，但不重写既有 Git tag 或 Release。
- 已记录技术债：公共文档定位器同时收到 `document` 与 `document_id` 时会静默优先前者，后续应统一增加一致性校验。
- 验证：完整测试 304 passed；当前源码 JSON-RPC、能力库临时开发版 MCP 和子代理实际调用三层均通过，普通引用、子文档汇总和仅块链接关系的正例可正确计数；临时 MCP 注册已关闭并删除，`dist/package.zip` 构建成功。
...
- 底层直接复用思源 `/api/asset/insertLocalAssets`，不经过官方 MCP；Bridge 不自行处理 Windows/macOS 路径转换、资源目录、哈希去重或重名改写。
...
- 写入前仍创建思源快照；上传后若插入或验证失败，只删除本批精确匹配的文档块，不自动删除可能被其他文档复用的附件。
- 三层行为验证均通过：当前源码 JSON-RPC 探针、能力库临时开发版 MCP、子代理实际调用都正确完成图片→文件→文件夹的顺序插入和读回验证，测试块已删除；临时 MCP 注册和唯一未引用测试附件也已清理。
- 自动化验证：附件相关测试 187 passed，完整测试 299 passed；插件 Bridge 副本同步与 `dist/package.zip` 构建成功。完整工具契约、实测事实和安全边界见 [`docs/ASSET_INSERTION_PLAN.md`](./ASSET_INSERTION_PLAN.md)。
