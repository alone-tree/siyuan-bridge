...
|---|---|---|---|
| 工具名称、schema、description | 单元测试 | `load` 后检查开发版工具面 | 默认不执行 |
| 单个读取/搜索工具行为 | 单元测试 | 调用开发版验证真实场景 | 默认不执行 |
| 局部报错文案或返回格式 | 单元测试 | 调用开发版验证受影响场景 | 默认不执行 |
| 跨多个工具或核心数据流 | 覆盖完整调用链 | 调用开发版验证完整真实流程 | 必须独立验证 |
| 复杂写入流程 | Fake Client 覆盖写入、读回、清理语义 | 调用开发版在临时测试文档完成同一流程 | 必须独立验证 |
