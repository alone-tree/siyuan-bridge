...
AI 可以按关键词搜索整个知识库，也可以浏览笔记本和文档目录。遇到长文时，它会先看到文章大纲，再按完整段落分段阅读，不会把内容从一句话中间截断。需要修改时，它能定位到具体段落，完成单段改写、连续多段重组、前后插入、整段删除和文末追加。可以在插件设置中打开“显示思源桥块序号”，让页面上的数字与 AI 说的“第 N 块”一致；改文档后序号会重算，AI 再次引用前会重新读取。
